...
class _SurveysScreenState extends ConsumerState<SurveysScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final allSurveys = allOffers.where((o) => o.type == OfferType.survey).toList();
    final available = allSurveys.where((o) => o.status == OfferStatus.available).toList();
    final mySurveys = allSurveys.where((o) => o.status != OfferStatus.available).toList();
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text('Share your opinion and earn quick points.', style: AppTypography.bodySmall),
              ),
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: const Icon(Icons.poll_rounded, color: AppColors.accent, size: 20),
          ),
  ...
  Widget _buildTabs() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _TabButton(label: 'Surveys', active: _tab == 0, onTap: () => setState(() => _tab = 0)),
            const SizedBox(width: 4),
            _TabButton(label: 'My Surveys', active: _tab == 1, onTap: () => setState(() => _tab = 1)),
          ],
        ),
      ),
    );
  ...
  Widget _buildAvailableList(List<OfferwallItemModel> surveys) {
    if (surveys.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.poll_outlined, size: 48, color: AppColors.textTertiary),
          const SizedBox(height: 12),
          Text('No surveys available right now', style: AppTypography.bodyMedium),
        ...
        itemBuilder: (context, i) {
          return AnimationConfiguration.staggeredList(
            ...
            child: SlideAnimation(
              verticalOffset: 30,
              child: FadeInAnimation(child: _SurveyCard(offer: surveys[i])),
            ),
          );
        },
      ),
  ...
  Widget _buildMyList(List<OfferwallItemModel> surveys) {
    if (surveys.isEmpty) {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.poll_outlined, size: 48, color: AppColors.textTertiary),
          const SizedBox(height: 12),
          Text('No surveys started yet', style: AppTypography.bodyMedium),
          const SizedBox(height: 4),
          Text('Start a survey from the list to track it here', style: AppTypography.bodySmall),
        ]),
      );
    }
...
class _SurveyCard extends StatelessWidget {
  ...
  const _SurveyCard({required this.offer});
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(offer.icon, color: AppColors.accent, size: 22),
          ),
          const SizedBox(width: 14),
          Expanded(
            child: Column(
