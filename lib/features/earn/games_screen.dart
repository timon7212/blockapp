...
class _GamesScreenState extends ConsumerState<GamesScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final allGames = allOffers.where((o) => o.type == OfferType.reachLevel || o.type == OfferType.installApp).toList();
    final availableGames = allGames.where((o) => o.status == OfferStatus.available).toList();
    final myGames = allGames.where((o) => o.status != OfferStatus.available).toList();
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text('Install games and reach milestones to earn big points.', style: AppTypography.bodySmall),
              ),
              ...
              if (_loading)
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.symmetric(horizontal: 24),
                    child: ShimmerCardList(count: 5, itemHeight: 80),
                  ),
                )
              else
                Expanded(
                  child: _tab == 0
                      ? _OfferList(offers: availableGames, emptyIcon: Icons.sports_esports_outlined, emptyText: 'No games available right now')
                      : _MyOfferList(offers: myGames, emptyIcon: Icons.sports_esports_outlined, emptyText: 'No games started yet'),
                ),
            ],
          ),
        ),
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: const Icon(Icons.sports_esports_rounded, color: AppColors.success, size: 20),
          ),
  ...
  Widget _buildTabs() {
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            _TabButton(label: 'Games', active: _tab == 0, onTap: () => setState(() => _tab = 0)),
            const SizedBox(width: 4),
            _TabButton(label: 'My Games', active: _tab == 1, onTap: () => setState(() => _tab = 1)),
          ],
        ),
      ),
    );
...
class _OfferList extends StatelessWidget {
  final List<OfferwallItemModel> offers;
  final IconData emptyIcon;
  final String emptyText;
  const _OfferList({required this.offers, required this.emptyIcon, required this.emptyText});
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(emptyText, style: AppTypography.bodyMedium),
        ...
        itemBuilder: (context, i) {
          return AnimationConfiguration.staggeredList(
            ...
            child: SlideAnimation(
              verticalOffset: 30,
              child: FadeInAnimation(child: _OfferCard(offer: offers[i])),
            ),
          );
        },
      ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Icon(offer.icon, color: AppColors.success, size: 22),
          ),
...
class _MyOfferList extends StatelessWidget {
  final List<OfferwallItemModel> offers;
  final IconData emptyIcon;
  final String emptyText;
  const _MyOfferList({required this.offers, required this.emptyIcon, required this.emptyText});
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(emptyText, style: AppTypography.bodyMedium),
            const SizedBox(height: 4),
            Text('Start an offer from the list to track it here', style: AppTypography.bodySmall),
          ],
        ),
      );
    }
